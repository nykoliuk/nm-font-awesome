...
function nm_font_awesome() {
    wp_enqueue_script( 'nm-fontawesome', 'https://use.fontawesome.com/releases/v5.6.3/js/all.js', array(), null );
    wp_enqueue_script( 'nm-fontawesome-shims', plugins_url( 'public/js/v4-shims.min.js', __FILE__ ), array(), null );
}
...
function add_nm_defer_attribute( $tag, $handle, $src ) {
    ...
    if ( 'nm-fontawesome-shims' === $handle ) {
        $tag = '<script defer src="' . esc_url( $src ) . '" crossorigin="anonymous"></script>';
    }
